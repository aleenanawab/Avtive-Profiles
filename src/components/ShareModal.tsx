  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200 text-left">
      <div className="relative w-full max-w-sm rounded-[32px] bg-white dark:bg-[#0A1128] border border-[#E2E8F0] dark:border-white/10 shadow-2xl p-6 text-center space-y-5 transition-colors">
        ...
        {/* Instagram-Style User Identity Header */}
        <div className="flex flex-col items-center space-y-2 pt-1">
          <div className="w-16 h-16 rounded-full p-0.5 bg-gradient-to-tr from-[#7EC384] to-[#1E3A8A] shadow-md">
            <img
              src={profile.avatar}
              alt={profile.name}
              className="w-full h-full object-cover rounded-full border border-white dark:border-[#0A1128]"
            />
          </div>
          <div>
            ...
            <p className="text-xs text-[#475569] dark:text-[#94A3B8] font-medium">
              {profile.company || profile.designation}
            </p>
          </div>
        </div>

        {/* Prominent Profile QR Code Card */}
        <div className="p-4 rounded-2xl bg-[#F8FAFC] dark:bg-[#0F172A] border border-[#E2E8F0] dark:border-white/10 inline-block shadow-inner">
          {qrUrl ? (
            ...
            <div className="w-48 h-48 mx-auto flex items-center justify-center text-xs text-[#94A3B8]">
              <QrCode className="w-10 h-10 animate-pulse text-[#94A3B8]" />
            </div>
          )}
          <p className="text-[11px] font-medium text-[#475569] dark:text-[#94A3B8] mt-3">
            Scan to view this profile
          </p>
        </div>

        {/* Simple Instagram-Style Share Action Buttons */}
        <div className="grid grid-cols-3 gap-2 pt-1">
          {/* 1. Copy Link */}
          <button
            onClick={handleCopy}
            className="flex flex-col items-center justify-center p-2.5 rounded-xl bg-[#F1F5F9] dark:bg-[#152238] hover:bg-[#E2E8F0] dark:hover:bg-[#1E3050] text-[#0A1128] dark:text-white border border-[#E2E8F0] dark:border-white/10 transition-colors active:scale-95 shadow-2xs"
          >
            {copied ? (
              <Check className="w-4 h-4 text-[#7EC384] mb-1" />
            ) : (
              <Copy className="w-4 h-4 text-[#0A1128] dark:text-white mb-1" />
            )}
          ...
          <button
            ...
            className="flex flex-col items-center justify-center p-2.5 rounded-xl bg-[#0A1128] hover:bg-[#152238] dark:bg-white dark:hover:bg-slate-100 text-white dark:text-[#0A1128] transition-colors active:scale-95 shadow-2xs"
          >
          ...
          <button
            ...
            className="flex flex-col items-center justify-center p-2.5 rounded-xl bg-[#F1F5F9] dark:bg-[#152238] hover:bg-[#E2E8F0] dark:hover:bg-[#1E3050] text-[#0A1128] dark:text-white border border-[#E2E8F0] dark:border-white/10 transition-colors active:scale-95 shadow-2xs"
          >
            <Download className="w-4 h-4 text-[#0A1128] dark:text-white mb-1" />
